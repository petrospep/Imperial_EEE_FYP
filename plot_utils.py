...
class Plot_Reproduce_Performance():
    def __init__(self, DIR, n_img_x=8, n_img_y=8, img_w=100, img_h=100,channels=3, resize_factor=1.0):
        self.DIR = DIR
    ...
    def save_images(self, images, name='result.jpg'):
        ...
        if self.channels==3:
            images = images.reshape(self.n_img_x * self.n_img_y, self.img_h, self.img_w, self.channels, order='F')
            scipy.misc.imsave(self.DIR + "/"+name, self._merge(images, [self.n_img_y, self.n_img_x]))

        else:
            images = images.reshape(self.n_img_x * self.n_img_y, self.img_h, self.img_w, order='F')
            scipy.misc.imsave(self.DIR + "/" + name, self._merge1(images, [self.n_img_y, self.n_img_x]))
    ...
    def _merge(self, images, size):
        ...
        img = np.zeros((h_ * size[0], w_ * size[1],self.channels))

        for idx, image in enumerate(images):
            i = int(idx % size[1])
            j = int(idx / size[1])

            image_ = scipy.misc.imresize(image, size=(w_,h_,3), interp='bicubic')

            img[j*h_:j*h_+h_, i*w_:i*w_+w_,:] = image_

        return img

    def _merge1(self, images, size):
        h, w = images.shape[1], images.shape[2]
        ...
        for idx, image in enumerate(images):
            ...
            image_ = scipy.misc.imresize(image, size=(w_,h_), interp='bicubic')
...
class Plot_Manifold_Learning_Result():
    ...
    def save_images(self, images, name='result.jpg'):
        images = images.reshape(self.n_img_x*self.n_img_y, self.img_h, self.img_w,3,order='F')
        scipy.misc.imsave(self.DIR + "/"+name, self._merge(images, [self.n_img_y, self.n_img_x]))
